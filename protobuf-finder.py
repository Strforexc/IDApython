...
class util:
    def syntax_highlight_proto(line):
        ...
        if str:
            colored_line = colored_line[:str.start()] + idaapi.COLSTR(colored_line[str.start():str.end()],
                                                                      idaapi.SCOLOR_DSTR) + colored_line[str.end():]
        # Variables
        var = re.search("\w+\s*(?==)", colored_line)
        if var:
            colored_line = colored_line[:var.start()] + idaapi.COLSTR(colored_line[var.start():var.end()],
                                                                      idaapi.SCOLOR_CODNAME) + colored_line[var.end():]
        # Objects
        obj = re.search("\w+\s*(?={)", colored_line)
        if obj:
            colored_line = colored_line[:obj.start()] + idaapi.COLSTR(colored_line[obj.start():obj.end()],
                                                                      idaapi.SCOLOR_IMPNAME) + colored_line[obj.end():]
        # Numbers
        num = re.search("\W\d+\W", colored_line)
        if num:
            colored_line = colored_line[:num.start()] + idaapi.COLSTR(colored_line[num.start():num.end()],
                                                                      idaapi.SCOLOR_DNUM) + colored_line[num.end():]
...
class ProtobufExtractor:
    ...
    def extract(self):
        ...
        while True:
            ...
            r = ida_bytes.bin_search(searchStartAddr, ida_ida.MAXADDR, bytes([0x2E, 0x70, 0x72, 0x6F, 0x74, 0x6F]),
                                     bytes([0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]), 1, 1)
            if r == idaapi.BADADDR:
            ...
            for j in range(64):
                try:
                    ...
                    if decode_varint128(protostring[1:])[0] == (j + 5) and is_valid_filename(
                            protostring[1 + 1:j + 6 + 1]):
